...
SO101SystemHardware::read(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/) {
  ...
  if (++read_counter % 100 == 0) {
    RCLCPP_INFO(rclcpp::get_logger("SO101HW_DEBUG"),
      "[READ] Positions: pan=%.3f lift=%.3f elbow=%.3f",
...
SO101SystemHardware::write(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/) {
  ...
  if (++counter % 100 == 0) {  // Каждые 100 циклов (1 раз в секунду при 100Hz)
    RCLCPP_INFO(rclcpp::get_logger("SO101HW_DEBUG"), 
      "[WRITE] Commands: pan=%.3f lift=%.3f elbow=%.3f",
